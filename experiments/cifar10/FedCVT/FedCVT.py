...
# lambda
Lambda1, Lambda2, Lambda3, Lambda4, Lambda5 = 0.1, 0.01, 0.01, 0.1, 0.1
...
class VFL_Client(nn.Module):
    def __init__(self):
        ...
        self.Linear_unique = nn.Linear(128 * 4 * 2, 64)
        self.Linear_common = nn.Linear(128 * 4 * 2, 64)
...
def train(model, epoch):
    ...
    for ep in range(epoch):
        ...
        for batch_idx in range(50):
            ...
            loss = (L_c_AB + L_pseudo_A + L_pseudo_B) + L_c_A + L_c_B + Lambda1 * L_dif_aligned + Lambda2 * L_dif_A_estimate + Lambda3 * L_dif_B_estimate + Lambda4 * L_sim_aligned_A + Lambda5 * L_sim_aligned_B
            # loss backward
            loss.backward()
            # update params
            optimizer.step()
