...
path = '/data0/data_sw/logs/VFL_base_log/cifar10/Ours/checkpoints/best_cifar10_' + str(currentDateAndTime) + '_' + str(
    LAMBDA)
...
handler = logging.FileHandler('/data0/data_sw/logs/VFL_base_log/cifar10/Ours/logs/' +
                              str(currentDateAndTime) + '_' + str(aligned_samples) + '.log')
    ...
    for ep in range(50):
        ...
        if TRAIN_SERVER:
            ...
            for batch_idx, (inputs, targets) in enumerate(train_aligned_loader):
                ...
                optimizer_server_align.zero_grad()
...
optimizer_server_align = torch.optim.Adam([{'params': client_A.parameters(), 'lr': 1e-3},
                                           {'params': client_B.parameters(), 'lr': 1e-3},
                                           {'params': server.parameters(), 'lr': 1e-5}])
