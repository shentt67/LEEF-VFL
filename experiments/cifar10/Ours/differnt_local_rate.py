...
path = '/data/data_sw/logs/VFL_base_log/cifar10/Ours/checkpoints/different_local_rate'+ str(currentDateAndTime) + '_' + '_best_cifar10_' + str(currentDateAndTime) + '_' + str(
    LAMBDA) + '_align_' + str(align_loss) + '_label_' + str(TRAIN_CLASSIFIER) + '_seed_' + str(seed)
...
handler = logging.FileHandler('/data/data_sw/logs/VFL_base_log/cifar10/Ours/logs/' +
    ...
    aligned_samples) + '_align_' + str(align_loss) + '_label_' + str(TRAIN_CLASSIFIER) + '_' + str(EPOCH) + '_different_local_rate_'+ str(RATE) + '_' + '.log')
...
# adjust sample rates, 0, 0.2, 0.4, 0.8, 1
# RATE = 0.2
train_data_A_with_aligned, _ = Data.random_split(train_data_A_with_aligned,
                                               [int(len(train_data_A_with_aligned) * RATE),
                                                len(train_data_A_with_aligned) - int(len(train_data_A_with_aligned) * RATE)])
train_data_B_with_aligned, _ = Data.random_split(train_data_B_with_aligned,
                                               [int(len(train_data_B_with_aligned) * RATE),
                                                len(train_data_B_with_aligned) - int(len(train_data_B_with_aligned) * RATE)])
    ...
    for ep in range(epoch):
        ...
        if TRAIN_SERVER:
            ...
            for batch_idx, (inputs, targets) in enumerate(train_aligned_loader):
                ...
                if align_loss:
                    # 7.29 after, all align
                    # if align_loss:
                    # align A
                    x_client_A_unaligned, _ = client_A(inputs_A[:, :, :, :16])
                    # calculate loss
                    # loss_align_A = LAMBDA * mmd(x_client_A_aligned, x_client_A_unaligned)
                    label_al = torch.concat([torch.ones(BATCH_SIZE), torch.zeros(BATCH_SIZE)], dim=0).type(torch.LongTensor).cuda()
                    k = (ep * (aligned_samples / BATCH_SIZE) + batch_idx) / (epoch * (aligned_samples / BATCH_SIZE))
                    lp = 1. * (2. / (1. + torch.exp(torch.tensor(-10. * k))) - 1)
                    lg_A = client_A.module.discriminator(client_A.module.flip_layer.apply(torch.cat([x_client_A_aligned, x_client_A_unaligned], dim=0), lp))
                    loss_align_A = LAMBDA * loss_func(lg_A, label_al)
                    # align B
                    x_client_B_unaligned, _ = client_B(inputs_B[:, :, :, 16:])
                    # calculate loss
                    # loss_align_B = LAMBDA * mmd(x_client_B_aligned, x_client_B_unaligned)
                    lg_B = client_B.module.discriminator(client_B.module.flip_layer.apply(torch.cat([x_client_B_aligned, x_client_B_unaligned], dim=0), lp))
                    loss_align_B = LAMBDA * loss_func(lg_B, label_al)
...
def test(client_A, client_B, server, epoch, classifier=False):
    ...
    with torch.no_grad():
        ...
        if classifier:
            ...
            logger.info(
                'Epoch: {}, test loss: {:.4f}, test accuracy_server: {:.4f}'.format(
                    epoch + 1,
                    test_loss_server,
                    test_acc_server))
